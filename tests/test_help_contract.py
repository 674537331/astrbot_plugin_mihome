...
class HelpContractTests(unittest.TestCase):
    ...
    def test_call_mihome_action_tool_exists_with_args_doc(self):
        tool_methods = [
            node for node in self.plugin_class.body
            if isinstance(node, ast.AsyncFunctionDef)
            and any(
                decorator_name(d) == "filter.llm_tool"
                and self._extract_llm_tool_name(d) == "call_mihome_action"
                for d in node.decorator_list
            )
        ]
        self.assertEqual(len(tool_methods), 1, "call_mihome_action 工具必须存在且唯一")
        docstring = ast.get_docstring(tool_methods[0]) or ""
        self.assertIn("Args:", docstring)
        self.assertIn("device_alias", docstring)
        self.assertIn("action", docstring)
